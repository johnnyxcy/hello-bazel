...
def get_files(build_env_input: Dict[str, Any]) -> List[EnvFile]:
    files: List[EnvFile] = []
...
def entry_points(path: List[str], **params: Any) -> importlib.metadata.EntryPoints:
    distributions = importlib.metadata.distributions(path=path)

    eps = itertools.chain.from_iterable(
        dist.entry_points
        for dist in distributions
    )
    return importlib.metadata.EntryPoints(eps).select(**params)
...
def main():
    ...
    sys._base_executable = str(pathlib.Path(sys._base_executable).resolve()) # type: ignore
